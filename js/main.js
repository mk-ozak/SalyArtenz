...
function initMap() {
	$(".map").each(function (e) {
		...
		let lat = mapOptions.lat ? mapOptions.lat : 49.4379661,
			lng = mapOptions.lng ? mapOptions.lng : 18.784908,
		...
		const gmap = new google.maps.Map(this, {
			...
			mapId: "7384bf17c1b95b96d1fccf39",
		});

		// prečítaj offset z options (ak nie je, nech je 0,0)
		const offset = mapOptions && mapOptions.offset ? mapOptions.offset : { x: 0, y: 0 };

		// posuň mapu po prvom vykreslení (iba raz)
		google.maps.event.addListenerOnce(gmap, "idle", function () {
			if (typeof offset.x === "number" && typeof offset.y === "number") {
				gmap.panBy(offset.x, offset.y);
			}
		});
